...
- `open [URL] [--new-tab]`：打开地址或启动浏览器。DOM 就绪后最多再等 3 秒 load 事件，没等到就先返回 `"loaded": false`，免得广告多的站要等十几秒。
- `snapshot [--in TARGET] [--max N]`：列出可见可操作元素并编号。
- `text [TARGET] [--max N]`：读取页面或元素文字。
- `click TARGET`、`hover TARGET`：点击或悬停元素。
- `fill TARGET TEXT`、`type TEXT`、`press KEY`、`select TARGET VALUE`：输入与键盘操作。
...
日志里的 `argv` 是原样记录的命令行，`fill` 填进去的密码等内容也会落在 `webctl.db` 里。这个库和 Cookie 在同一个目录，保护级别一致；不想记就设 `WEBCTL_LOG=0`。
