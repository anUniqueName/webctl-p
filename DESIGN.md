...
- 不内置识别验证码内容、自动解题的逻辑：不从截图里算拼图缺口，不按验证类型自动选操作；`open` 等命令认出拦截页也只报告类型，不去处理。不改浏览器指纹。
...
- 全局参数 `--session <名字>`（默认 `default`，也可用环境变量 `WEBCTL_SESSION`）。每个会话一个状态文件 `<数据目录>/sessions/<名字>.json`：
  ...
  ```json
  {"endpoint": "http://127.0.0.1:9333", "launched": true, "pid": 1234, "current_target": "<targetId>"}
  ```
...
1. 给了全局参数 `--cdp <端口|http地址|ws地址>`：连接它，写入状态文件（`launched: false`）。
2. 否则读状态文件；endpoint 可连通（`/json/version` 1 秒内返回）就直接用。
3. 否则启动 Chrome：
   - 可执行文件查找顺序：环境变量 `WEBCTL_CHROME` → Windows 常见路径（`%ProgramFiles%`、`%ProgramFiles(x86)%`、`%LOCALAPPDATA%` 下的 `Google\Chrome\Application\chrome.exe`）→ Edge（`Microsoft\Edge\Application\msedge.exe`）→ macOS/Linux 常见路径。都找不到就报错，提示设置 `WEBCTL_CHROME`。
   - 端口：绑定 `127.0.0.1:0` 拿到一个空闲端口后释放。
   - 启动参数：`--remote-debugging-port=<端口> --user-data-dir=<配置目录> --no-first-run --no-default-browser-check --hide-crash-restore-bubble`；全局参数 `--headless` 时再加 `--headless=new`。**不加** `--no-sandbox`，不加 `--enable-automation`，也不加任何用来隐藏自动化特征的参数。
   ...
   - 轮询 `/json/version`，最多等 15 秒。
   - 已知情况：同一个配置目录已经有 Chrome 在运行时（比如状态文件被删了），新启动的进程会把请求转交给已有进程然后退出，调试端口不会生效。这时报错提示：关闭使用该配置目录的 Chrome 后重试，或用 `--cdp` 连接。
...
1. JS：找到元素 → `scrollIntoView({block: 'center', inline: 'center'})` → 取 `getBoundingClientRect()` 的中心点。元素在同源 iframe 里时，坐标加上各级 iframe 在上一级视口中的偏移（iframe 的 rect.left/top 加上 border 宽度和 padding）。
2. 遮挡检查：在元素所在的文档里，用本地坐标调 `elementFromPoint`。命中的节点既不是该元素也不在该元素内部时，报错 `被 <div#consent.mask> 遮挡`（遮挡元素有编号就一并给出），加 `--force` 时跳过检查。
...
- `find <IMAGE>`：`Page.captureScreenshot` 截当前视口 → `vision.rs` 解码成灰度图 → 两级 ZNCC 模板匹配（先按模板短边/8 降采样粗匹配，非极大值抑制后留至少 32 个候选，回原图 ±(factor+1) 窗口精修）。粗匹配只挑候选，阈值比 `--threshold` 低 0.4：目标左上角和降采样网格对不齐时粗分数会掉，实测 42x25 的文字按钮在 9 种对齐偏移下粗分数 0.67–1.0，用原阈值筛会漏掉一大半位置。是否匹配由原图精修的分数按 `--threshold` 判定。窗口统计量走积分图，耗时跟模板大小基本无关。匹配分数 0–1，`--threshold` 默认 0.8。截图是设备像素，输出坐标按视口宽高比换算回 CSS 像素，x/y 指匹配中心。**不做缩放不变性**：模板要和页面同一缩放、同一 devicePixelRatio，从 `webctl screenshot` 截的图里裁最稳。
...
- `fill <TARGET> <TEXT>`：目标本身不是输入框、但在 `<label>` 里时，改为操作标签关联的输入框（`text=用户名` 找到的常是标签文字）。JS 聚焦元素，焦点没落到目标或它的子元素上就报错：不是输入框时接着输入会打进之前有焦点的那个框。报错写明原因：焦点被页面转到了别的元素时（点搜索框弹出另一个输入框）写出那个元素，如 `<input#real>`；目标不可见、已禁用时直说；其余情况说目标不是输入框。默认先全选已有内容——用 CDP 按键 `Control+A` 并带参数 `commands: ["selectAll"]`（跨平台可靠，对 React 受控输入框也有效）——再 `Input.insertText {text}`。TEXT 为空时全选后发一次 Backspace。`--append` 时不全选，用 JS 把光标移到末尾后插入。完成后读出元素当前的 value（contenteditable 读 innerText）放进输出，然后做变化检测。
...
- 单元测试：按键解析（`Control+A`、`Shift+Tab`、`Enter`）、目标参数解析（`e3`/`@e3`/`text=`/CSS）、URL 补全、行集合差异、模板匹配（按钮样式的模板放在降采样网格的 9 种偏移上都能找到）。
